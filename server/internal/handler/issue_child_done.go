		...
		if !hasCancelled {
			...
			if !batch {
				...
				content = fmt.Sprintf(
					...
					mentionPrefix, identifier, childID, title, lastAction, warning, parentContinuationInstruction(parentID),
				)
			} else {
				content = fmt.Sprintf(
...
func stageAdvanceInstruction(nextStage int32, parentID string, stageCancelled int, scopeCancelled bool) string {
	...
	if nextStage > 0 {
		...
		instruction = fmt.Sprintf(" %s this stage does not mean the whole issue is done. %s", verb, parentContinuationInstruction(parentID))
	}
	if !scopeCancelled {
		return instruction
	}
